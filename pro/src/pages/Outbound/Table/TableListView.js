...
import { deleteItem } from '@/services/Outbound/index';
...
@connect(({ outbound, loading }) => {
  return {
    list: outbound.list,
    pagination: outbound.pagination,
     density:global.density || 8,
    loading: loading.effects['outbound/fetch'] || false,
  };
})
class TableListView extends React.Component {
  ...
  <AuthorizedButton
          ...
          onClick={() => {
            this.props.history.push(`/stockout/outbound/${record.id}`);
          }}
        />
        <AuthorizedDetailButton
          ...
          permissionKey={record.billstatus}
          onClick={() => {
            this.props.history.push(`/stockout/outbound/detail/${record.id}`);
          }}
        />
        <AuthorizedDelete
          title='删除'
      ...
      <div className={styles.tableView} ref={(el) => { this.parent = el; }}>
        <div className={styles.btnGroup} ref={(el) => { this.btnGroup = el; }}>
          <Button
            ...
            onClick={() => {
              this.props.history.push('/stockout/outbound/add')
            }}
          >
            新增
          </Button>
