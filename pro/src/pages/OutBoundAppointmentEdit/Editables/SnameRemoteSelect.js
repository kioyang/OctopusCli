...
class SnameRemoteSelect extends React.Component {
  ...
  fetchList = value => {
    ...
    let url = '/portal/rs/stock/findByBatchNo?page=1&limit=1000&keyword=' + value;
    if (!value) {
      url = '/portal/rs/stock/findByBatchNo?page=1&limit=1000'
    }
    fetch(url)
      ...
      >
        ...
        {data && data.map(d => (
          <Option key={d.id} value={d.id}>
            <span style={ {width: 100, display: 'inline-block'} }>{d.commoditycode}</span>
            <span style={ {display: 'inline-block', width: 300} }>{d.commodityName}</span>
            {d.materialmodel}&nbsp;&nbsp;<span>{d.commoditySpecification}</span></Option>
        ))}
      </Select>
    );
  }
