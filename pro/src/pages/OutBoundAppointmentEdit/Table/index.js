      ...
      tableForm.validateFields().then((values) => {
        this.getFormData()
          .then((params) => {
            ...
            const pms = {...result,id: this.id && this.id !== 'add' ? this.id : undefined,operStatus:type};
            dispatch({
              type: 'outboundAppointmentEdit/saveObject',
              payload: { params: { ...pms } }
            })
              .then((res) => {
                resolve(res);
...
// indexpage
export default connect(({ outboundAppointmentEdit , loading }) => {
  return {
    editList: outboundAppointmentEdit.editList, // 为了提交获取editList
    loading: loading.effects['outboundAppointmentEdit/saveObject'] || false,
  }
})(Index);
