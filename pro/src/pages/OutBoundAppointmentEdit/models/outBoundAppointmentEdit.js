...
import { getList, saveService, submitService } from '@/services/OutboundAppointmentEdit'

const pageSize = 15;
export default {
  namespace: 'outboundAppointmentEdit', // combineReducer的键
  // 默认值
  state: {
    // 列表数据
    list: [],
  ...
  effects: {
    ...
    *fetch({ payload }, { call, put, select }) {
      const params = yield select(({ outboundAppointmentEdit }) => {
        return outboundAppointmentEdit.queryParams;
      });
    ...
    *addRow({ payload }, { call, put, select }) {
      const { row } = payload;
      const editList = yield select(({ outboundAppointmentEdit }) => {
        return outboundAppointmentEdit.editList;
      }) || [];
      // editList长度作为onlyid
