      ...
      tableForm.validateFields().then((values) => {
        this.getFormData()
          .then((params) => {
            ...
            dispatch({
              type: 'stockInAppointmentEdit/saveObject',
              payload: { params: { ...pms } }
            })
              .then((res) => {
                resolve(res);
...
export default connect(({ stockInAppointmentEdit, loading }) => {
  return {
    editList: stockInAppointmentEdit.editList, // 为了提交获取editList
    loading: loading.effects['stockInAppointmentEdit/saveObject'] || false,
  }
})(Index);
