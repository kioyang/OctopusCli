...
import { getList, saveService, submitService } from '@/services/stockInAppointmentDetail'

const pageSize = 15;
export default {
  namespace: 'stockInAppointmentEdit', // combineReducer的键
  // 默认值
  state: {
    // 列表数据
    list: [],
  ...
  effects: {
    ...
    *addRow({ payload }, { call, put, select }) {
      const { row } = payload;
      const editList = yield select(({ stockInAppointmentEdit }) => {
        return stockInAppointmentEdit.editList;
      }) || [];
      // editList长度作为onlyid
