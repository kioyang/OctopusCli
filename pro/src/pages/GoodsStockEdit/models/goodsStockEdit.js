...
import { getList, saveService, submitService } from '@/services/GoodsStockEdit'

const pageSize = 15;
export default {
  namespace: 'goodsStockEdit', // combineReducer的键
  // 默认值
  state: {
    // 列表数据
    list: [],
  ...
  effects: {
    ...
    *fetch({ payload }, { call, put, select }) {
      const params = yield select(({ goodsStockEdit }) => {
        return goodsStockEdit.queryParams;
      });
    ...
    *addRow({ payload }, { call, put, select }) {
      const { row } = payload;
      const editList = yield select(({ goodsStockEdit }) => {
        return goodsStockEdit.editList;
      }) || [];
      // editList长度作为onlyid
