      ...
      tableForm.validateFields().then((values) => {
        this.getFormData()
          .then((params) => {
            ...
            dispatch({
              type: 'goodsStockEdit/saveObject',
              payload: { params: { ...pms } }
            })
              .then((res) => {
                resolve(res);
...
// indexpage
export default connect(({ goodsStockEdit , loading }) => {
  return {
    editList: goodsStockEdit.editList, // 为了提交获取editList
    loading: loading.effects['goodsStockEdit/saveObject'] || false,
  }
})(Index);
