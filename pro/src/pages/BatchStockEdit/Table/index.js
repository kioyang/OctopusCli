      ...
      tableForm.validateFields().then((values) => {
        this.getFormData()
          .then((params) => {
            ...
            dispatch({
              type: 'batchStockEdit/saveObject',
              payload: { params: { ...pms } }
            })
              .then((res) => {
                resolve(res);
...
// indexpage
export default connect(({ batchStockEdit , loading }) => {
  return {
    editList: batchStockEdit.editList, // 为了提交获取editList
    loading: loading.effects['batchStockEdit/saveObject'] || false,
  }
})(Index);
