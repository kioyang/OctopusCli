...
import { getList, saveService, submitService } from '@/services/BatchStockEdit'

const pageSize = 15;
export default {
  namespace: 'batchStockEdit', // combineReducer的键
  // 默认值
  state: {
    // 列表数据
    list: [],
  ...
  effects: {
    ...
    *fetch({ payload }, { call, put, select }) {
      const params = yield select(({ batchStockEdit }) => {
        return batchStockEdit.queryParams;
      });
    ...
    *addRow({ payload }, { call, put, select }) {
      const { row } = payload;
      const editList = yield select(({ batchStockEdit }) => {
        return batchStockEdit.editList;
      }) || [];
      // editList长度作为onlyid
