...
import { getList, saveService, submitService } from '@/services/OutboundEdit'

const pageSize = 15;
export default {
  namespace: 'outboundEdit', // combineReducer的键
  // 默认值
  state: {
    // 列表数据
    list: [],
  ...
  effects: {
    ...
    *fetch({ payload }, { call, put, select }) {
      const params = yield select(({ outboundEdit }) => {
        return outboundEdit.queryParams;
      });
    ...
    *addRow({ payload }, { call, put, select }) {
      const { row } = payload;
      const editList = yield select(({ outboundEdit }) => {
        return outboundEdit.editList;
      }) || [];
      // editList长度作为onlyid
